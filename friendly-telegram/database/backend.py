...
class CloudBackend:
    ...
    async def _find_data_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if dialog.name == f"🌇 UMOD -{self._me.user_id}-𝙳𝚊𝚝𝚊" and dialog.is_channel:
    ...
    async def _find_asset_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if (
                dialog.name == f"🌇 UMOD -{self._me.user_id}- Base"
    ...
    async def do_upload(self, data):
        ...
        if newmsg:
            await self._client.send_message(self.db, "<b>🌇 Shunchaki log uchun guruh.</b>\n🌉 Guruhni oʻchirmang!")
